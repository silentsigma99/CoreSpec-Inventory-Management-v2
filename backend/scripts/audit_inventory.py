...
Run from backend directory: python scripts/audit_inventory.py [csv_files...]
    ...
    if discrepancies:
        ...
        for d in discrepancies:
            ...
            if d['diff_csv_vs_db'] < 0:
                print(f"  ⚠️  ISSUE: DB has {abs(d['diff_csv_vs_db'])} LESS than CSV expected")
                if d['tx_net'] < d['csv_total']:
                    print(f"      → Possible cause: Import was not run or partially failed")
            elif d['diff_csv_vs_db'] > 0:
                print(f"  ⚠️  ISSUE: DB has {d['diff_csv_vs_db']} MORE than CSV expected")
                if d['tx_net'] > d['csv_total']:
                    print(f"      → Possible cause: Import was run multiple times")

            if d['diff_tx_vs_db'] != 0:
                print(f"  ⚠️  ISSUE: Transaction net ({d['tx_net']}) != DB qty ({d['db_qty']})")
